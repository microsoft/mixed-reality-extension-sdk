...
export default class PhysicsLocalVsRemoteTest2 extends Test {
	...
	public async run(root: MRE.Actor): Promise<boolean> {
		...
		if (this.app.context.users.length > 0) {
			...
			this.spawnBall(root, 0.0, 0.3, this.redMat, userId);
			this.spawnBall(root, 0.0, 0.5, this.redMat, userId);
			this.spawnBall(root, 0.0, 0.7, this.redMat, userId);
		}
